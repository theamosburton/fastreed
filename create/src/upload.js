...
createImageElement(imgURL, ulink, u) {
...
async fetchAndDisplayMedia(u) {
  ...
  if (type == 'image') {
    ...
    this.createImageElement.call(this, imgURL, ulink, u);
  } else if (type == 'video') {
    const response = await fetch(ulink);
    const blob = await response.blob();
    var videoURL = URL.createObjectURL(blob);
    this.createVideoElement.call(this, videoURL, ulink, u);
  }
}
...
showUploads() {
  return new Promise((resolve) => {
    ...
    for (let u = 0; u < this.uploadsCount; u++) {
      promises.push(this.fetchAndDisplayMedia.call(this, u));
    }
